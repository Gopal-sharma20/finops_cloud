...
import { generateUtilizationData } from "@/lib/aws/transform"
...
export default function AWSCloudPageMock() {
  const [selectedRegion, setSelectedRegion] = useState("all")
  const [selectedTimezone, setSelectedTimezone] = useState("all")
  const [selectedService, setSelectedService] = useState("all")

  // MOCK DATA ONLY - No API calls
  console.log("🎭 MOCK DATA PAGE - Using only mock data for comparison")
  ...
  // Always use mock regions
  const realRegions = useMemo(() => {
    console.log("🎭 Using MOCK regions data")
    return awsRegions
  }, [])

  // Always use mock recommendations
  const realRecommendations = useMemo(() => {
    console.log("🎭 Using MOCK recommendations data")
    return awsRecommendations
  }, [])

  // Always use mock cost trends
  const realCostTrends = useMemo(() => {
    console.log("🎭 Using MOCK cost trends data")
    return costTrends
  }, [])

  // Always use mock utilization
  ...
  // Mock refetch functions (do nothing)
  const refetchCost = () => {
    console.log("🎭 MOCK: Refresh button clicked (no real data fetch)")
  }
  const refetchAudit = () => {}
